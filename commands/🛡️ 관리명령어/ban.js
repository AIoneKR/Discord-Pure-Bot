    ...
    run: async (client, message) => {
      ...
      if(message.member.permissions.has("BAN_MEMBERS")){
        ...
        if (user) {
         ...
         embed.setTitle(`밴`)
         embed.setDescription(`${emojil}` + " 성공적으로 밴이 완료되었습니다.")
        ...
        console.log(`> ${message.guild.name} < | ${message.channel.name} | ${message.author.tag} (${message.author.id}) /밴 사용 > ${user.tag}를 밴하였습니다`)
              })
              .catch(err => {
				  let embed = new Discord.MessageEmbed()
         embed.setColor(0xff0d10)
         embed.setTitle(`ERROR!`)
         embed.setDescription("봇에게 밴권한이 없습니다!\n" + err)
         embed.setTimestamp()
         embed.setFooter(process.env.botname + ` ${process.env.v}`, client.user.displayAvatarURL())
        ...
        message.react('692644452401020958')
		  ...
		  let embed = new Discord.MessageEmbed()
         embed.setColor(0xff0d10)
         embed.setTitle(`Ban Permission ERROR!`)
